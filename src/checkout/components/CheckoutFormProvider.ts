...
function maybeSdk(state: CheckoutState): stripeJs.StripeCheckoutFormSdk | null {
  if (state.type === 'success' || state.type === 'loading') {
    return state.sdk as stripeJs.StripeCheckoutFormSdk | null
  }
  return null
}

export const CheckoutFormProvider = defineComponent({
  inheritAttrs: false,
  ...
  props: {
    ...
    options: {
      type: Object as PropType<stripeJs.StripeCheckoutFormSdkOptions>,
      required: true,
    },
  },
  setup(props, { slots }) {
    ...
    watch(parsed, (currentParsed, _, onCleanup) => {
      ...
      const init = (loadedStripe: stripeJs.Stripe) => {
        ...
        initCalled = true
        const sdk = loadedStripe.initCheckoutFormSdk(props.options)
          ...
          .then((result) => {
            ...
            if (result.type === 'success') {
              ...
              sdk.on('change', (session) => {
                if (state.value.type === 'success' && state.value.sdkKind === 'form') {
    ...
    let previousAppearance = createSnapshot(props.options.appearance)
    watch(() => props.options.appearance, (appearance) => {
    ...
    let previousFonts = createSnapshot(props.options.fonts)
    watch(() => props.options.fonts, (fonts) => {
