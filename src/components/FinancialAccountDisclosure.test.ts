...
function mockSuccessfulStripeJsCall() {
  return {
    ...mocks.mockStripe(),
    createFinancialAccountDisclosure: vi.fn(() =>
      Promise.resolve({
...
describe('financialAccountDisclosure', () => {
  ...
  it('should emit load event when disclosure loads', async () => {
    const wrapper = render(FinancialAccountDisclosure, {
      props: {
        stripe: mockStripePromise,
      },
    })
  ...
  it('does not mount until Stripe instance is available', async () => {
    ...
    await nextTick()
    await waitFor(() => {
      expect(mockStripe.createFinancialAccountDisclosure).toHaveBeenCalled()
    })
  })
  ...
  it('recreates disclosure when options change', async () => {
    ...
    const initialCallCount = mockStripe.createFinancialAccountDisclosure.mock.calls.length
