...
export const EmbeddedCheckoutContextKey = Symbol(
  'embedded checkout context',
) as InjectionKey<EmbeddedCheckoutContextValue>
...
export const EmbeddedCheckoutProvider = defineComponent({
  ...
  setup(props, { slots }) {
    ...
    const embeddedCheckoutPromise = shallowRef<Promise<void> | null>(null)
    ...
    watch(
      [parsed, () => props.options],
      ([currentParsed, currentOptions], _, onCleanup) => {
        let cancelled = false
        onCleanup(() => {
          cancelled = true
        })
