...
- e8330ec: Add `LinkSignupElement` to `vue-stripe` and `vue-stripe/checkout`. It requires Stripe beta access, and the checkout version works only inside `CheckoutElementsProvider`.

  Raise the `@stripe/stripe-js` peer range to `>=9.16.0 <10.0.0`.

## 3.0.0

### Major Changes

- 589636d: Sync with `@stripe/react-stripe-js@6.8.1` and upgrade to `@stripe/stripe-js` v9.
